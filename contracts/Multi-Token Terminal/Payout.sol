...
import 'https://github.com/jbx-protocol/juice-contracts-v2/blob/main/contracts/libraries/JBTokens.sol';
...
import './SingleTokenPaymentTerminal.sol';
  ...
  IJBPayoutRedemptionPaymentTerminal
{
  ...
  */
  function distributePayoutsOf(
    ...
    return _distributePayoutsOf(_projectId, _token, _amount, _currency, _minReturnedTokens, _memo);
  }
  ...
  */
    ...
    returns (uint256 netDistributedAmount)
  {
    ...
    return _useAllowanceOf(_projectId, _token, _amount, _currency, _minReturnedTokens, _beneficiary, _memo);
  }
  ...
  */
  function migrate(uint256 _projectId, IJBPaymentTerminal _to, address _token)
    external
    virtual
    override
    ...
    returns (uint256 balance)
  {
    ...
    if (_to.acceptsToken(_token, _projectId)) revert TERMINAL_TOKENS_INCOMPATIBLE();
    ...
    if (balance > 0) {
      ...
      _to.addToBalanceOf{value: _payableValue}(_projectId, balance, _token, '', bytes(''));
    }
  ...
  */
    ...
    )
  {
    ...
    for (uint256 _i = 0; _i < _heldFeeLength; ) {
      ...
      // Process the fee.
      _processFee(_amount, _heldFees[_i].beneficiary, _token);
    ...
    // Scoped section prevents stack too deep. `_delegate` only used within scope.
    {
      ...
      if (_delegate != IJBRedemptionDelegate(address(0))) {
        JBDidRedeemData memory _data = JBDidRedeemData(
          ...
          _tokenCount,
          JBTokenAmount(_token, reclaimAmount, decimals, currency),
          _beneficiary,
          _memo,
          _metadata
        );
  ...
  function _distributePayoutsOf(
    ...
    // Scoped section prevents stack too deep. `_feeDiscount`, `_feeEligibleDistributionAmount`, and `_leftoverDistributionAmount` only used within scope.
    {
      ...
      uint256 _feeDiscount = fee == 0 || isFeelessAddress[msg.sender]
        ...
        : _currentFeeDiscount(_projectId, _token);
  ...
  function _useAllowanceOf(
    ...
    // Scoped section prevents stack too deep. `_fee`, `_projectOwner`, `_feeDiscount`, and `_netAmount` only used within scope.
    {
      ...
      _fee = _feeDiscount == JBConstants.MAX_FEE_DISCOUNT
        ? 0
        : _takeFeeFrom(_projectId, _token, _fundingCycle, _distributedAmount, _projectOwner, _feeDiscount);
  ...
  */
  function _takeFeeFrom(
    ...
    if (_fundingCycle.shouldHoldFees()) {
      ...
      // Process the fee.
      _processFee(feeAmount, _beneficiary, _token); // Take the fee.
  ...
  */
  function _processFee(uint256 _amount, address _beneficiary, address _token) internal {
    // Get the terminal for the protocol project.
    IJBPaymentTerminal _terminal = directory.primaryTerminalOf(_PROTOCOL_PROJECT_ID, _token);
  ...
  */
  function _pay(
    ...
    // Scoped section prevents stack too deep. `_delegate` and `_tokenCount` only used within scope.
    {
      ...
      JBTokenAmount memory _bundledAmount = JBTokenAmount(_token, _amount, decimals, currency);
  ...
  */
  function _currentFeeDiscount(uint256 _projectId, address _token) internal view returns (uint256 feeDiscount) {
    // Can't take a fee if the protocol project doesn't have a terminal that accepts the token.
    if (directory.primaryTerminalOf(_PROTOCOL_PROJECT_ID, _token) == IJBPaymentTerminal(address(0)))
