...
import 'https://github.com/jbx-protocol/juice-contracts-v2/blob/main/contracts/JBTokenStore.sol';
/**
  @notice
...
contract JBSingleTokenPaymentTerminalStore is ReentrancyGuard, IJBSingleTokenPaymentTerminalStore {
  ...
  */
  ...
  IJBTokenStore public immutable token;
  ...
  */
  function currentReclaimableOverflowOf(
    ...
    JBFundingCycle memory _fundingCycle = fundingCycleStore.currentOf(param._projectId);
    ...
    uint256 _currentOverflow = _useTotalOverflow
      ? _currentTotalOverflowOf(param._projectId, _terminal.decimals(), _terminal.currency())
      : _overflowDuring(_terminal, param._projectId, _fundingCycle, _terminal.currency());

    // If there's no overflow, there's no reclaimable overflow.
    if (_currentOverflow == 0) return 0;

    // Get the number of outstanding tokens the project has.
    uint256 _totalSupply = IJBController(directory.controllerOf(param._projectId))
      .totalOutstandingTokensOf(param._projectId, _fundingCycle.reservedRate());

    // Can't redeem more tokens that is in the supply.
    if (param._tokenCount > _totalSupply) return 0;
  ...
  */
  function currentReclaimableOverflowOf(
    ...
    // Can't redeem more tokens that is in the supply.
    if (param._tokenCount > param._totalSupply) return 0;
    ...
    // Return the reclaimable overflow amount.
    return
      _reclaimableOverflowDuring(param, _fundingCycle, _overflow);
  }
  ...
  */
    ...
    )
  {
    ...
    tokenCount = PRBMath.mulDiv(_amount.value * 10**_decimals, _weight, _weightRatio);
  ...
  */
  ...
  {
    // Get a reference to the project's current funding cycle.
    fundingCycle = fundingCycleStore.currentOf(param._projectId);
    ...
    // Scoped section prevents stack too deep. `_reclaimedTokenAmount`, `_currentOverflow`, and `_totalSupply` only used within scope.
    {
      ...
      // Another scoped section prevents stack too deep. `_token`, `_decimals`, and `_currency` only used within scope.
      {
        ...
        _currentOverflow = fundingCycle.useTotalOverflowForRedemptions()
          ? _currentTotalOverflowOf(param._projectId, _decimals, _currency)
          : _overflowDuring(
            IJBSingleTokenPaymentTerminal(msg.sender),
            param._projectId,
            fundingCycle,
        ...
        // Get the number of outstanding tokens the project has.
        _totalSupply = IJBController(directory.controllerOf(param._projectId)).totalOutstandingTokensOf(
          param._projectId,
          fundingCycle.reservedRate()
        );

        // Can't redeem more tokens that is in the supply.
        if (param._tokenCount > _totalSupply) revert INSUFFICIENT_TOKENS();
    ...
    if (reclaimAmount > balanceOf[IJBSingleTokenPaymentTerminal(msg.sender)][param._projectId])
      revert INADEQUATE_PAYMENT_TERMINAL_STORE_BALANCE();

    // Remove the reclaimed funds from the project's balance.
    if (reclaimAmount > 0)
      balanceOf[IJBSingleTokenPaymentTerminal(msg.sender)][param._projectId] =
        balanceOf[IJBSingleTokenPaymentTerminal(msg.sender)][param._projectId] -
        reclaimAmount;
  }
  ...
  */
  function _reclaimableOverflowDuring(
    ...
    if (param._tokenCount == param._totalSupply) return _overflow;

    // Use the ballot redemption rate if the queued cycle is pending approval according to the previous funding cycle's ballot.
    uint256 _redemptionRate = fundingCycleStore.currentBallotStateOf(param._projectId) ==
    ...
    // If holder trys to reclaim partial balance, revert.
