  ...
  IJBPayoutRedemptionPaymentTerminal
{
  ...
  */
    ...
    returns (uint256)
  {
    ...
    uint256 _adjustedOverflow = (18 == 18)
      ? _overflow
      : JBFixedPointNumber.adjustDecimals(_overflow, 18, 18);
    ...
    return
      (currency == JBCurrencies.ETH)
        ...
        : PRBMath.mulDiv(
          _adjustedOverflow,
          10**18,
          prices.priceFor(currency, JBCurrencies.ETH, 18)
        );
  }
  ...
  */
  constructor(
    ...
    payable
    JBSingleTokenPaymentTerminal(_token, _currency)
  ...
  /**
    ...
    @param _id The amount of terminal tokens being received, as a fixed point number with the same amount of decimals as this terminal. If this terminal's token is ETH, this is ignored and msg.value is used in its place.
  ...
  */
  function pay(
    ...
    if (token != JBTokens.ETH) {
      ...
      _transferFrom(msg.sender, payable(address(this)), _id);
    }
  ...
  */
  ...
  /** 
    ...
    @param _id The amount of the transfer, as a fixed point number with the same number of decimals as this terminal.
  */
  function _beforeTransferTo(address _to, uint256 _id) internal virtual;
    ...
    // Scoped section prevents stack too deep. `_delegate` only used within scope.
    {
      ...
      if (_delegate != IJBRedemptionDelegate(address(0))) {
        JBDidRedeemData memory _data = JBDidRedeemData(
          ...
          _tokenCount,
          JBTokenAmount(token, reclaimAmount, 18, currency),
          _beneficiary,
          _memo,
          _metadata
        );
  ...
  */
  function _pay(
    ...
    // Scoped section prevents stack too deep. `_delegate` and `_tokenCount` only used within scope.
    {
      ...
      JBTokenAmount memory _bundledAmount = JBTokenAmount(token, 1, 18, currency);
