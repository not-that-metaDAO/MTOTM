// This script can be used to deploy the "Token" and "ERC20Terminal" contracts while launching a MTOTM project using ethers.js library.
...
//var Token;
var Terminal;
let ETHTerminal = '0x765A8b9a23F58Db6c8849315C04ACf32b2D55cF8';
let ETH         = '0x000000000000000000000000000000000000EEEe';
...
(async () => {
    try {
        ...
        const result = await deploy('Token', ['Mike', "T"])
        ...
        const project = await launch('JBController', [owner,['QmbH96jj8RTJgC9526RdsFs5dPvcsRfiRuD9797JXzcvbw', 0], //IPFS Hash
            [0,BigInt(1000000000000000000000000),0,"0x0000000000000000000000000000000000000000"], //Token amount per ETH
            [[false,false],2000,0,0,false,false,false,false,false,false,true,false,false,false,false,false,"0x0000000000000000000000000000000000000000"], //Funding cycle metadata (reserve amt, redemption amt)
             0, //Timestamp
            [
            ...
            [
                [ETHTerminal,ETH,BigInt(100000000000000000000000000000),1,0,0],
                [Terminal,Token,BigInt(100000000000000000000000000000),1,0,0]
            ], //Provides routing from terminal to split number provided in terminal deploy
            [ETHTerminal,Terminal],'',]  //Terminals
         ) 
